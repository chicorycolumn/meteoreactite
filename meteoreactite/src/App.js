...
function SuperTable(props) {
	...
	console.log(newData);
	function handleFoundButton(event) {
		event.preventDefault();
		console.log(props.data, newData);
		if (Array.isArray(newData)) {
			newData = props.data.filter((meteor) => meteor.fall === "Found");
			//set state in app
		} else {
	...
	const columns = [
		{
			...
			columns: [
				...
				},
				{
					Header: (
						<div>
							<button onClick={handleFoundButton}>Found</button>
							{/* <button onClick={handleFellButton}>Fell</button> */}
						</div>
					),
					accessor: "fall"
				},
...
class App extends React.Component {
	...
	componentDidMount() {
		fetch("https://data.nasa.gov/resource/gh4g-9sfh.json?$mass > 25000000")
			.then((response) => {
